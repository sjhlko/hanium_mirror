...
const parseRealTime = async (openApiResult, lineId) => {
  ...
  if (Array.isArray(realtimeRows)) {
    ...
    for (let i in realtimeRows) {
      const line = await lineServiceInstance.getLineByLineId(realtimeRows[i].subwayId._text);
      let realTimeObject = {
        lineName: line.dataValues.line_name,
    ...
    const realTimeObject = {
      lineName: lineServiceInstance.getLineByLineId(realtimeRows.subwayId._text),
